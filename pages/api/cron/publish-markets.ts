...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const authSecret = req.query.secret || req.headers.authorization?.split(" ")[1];
  ...
  try {
    ...
    for (const market of openMarkets) {
      ...
      const buttons = {
        reply_markup: {
          inline_keyboard: [[
            { text: "✅ YES", url: `${process.env.BOT_WEB_URL}/trade/${market.id}?side=yes` },
            { text: "❌ NO", url: `${process.env.BOT_WEB_URL}/trade/${market.id}?side=no` }
          ]]
        },
