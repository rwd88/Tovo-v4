      ...
      <div className={`${styles.page} ${geistSans.variable} ${geistMono.variable}`}>
        <main className={styles.main}>
          ...
          <div className="w-full">
            <h2 className="text-2xl font-bold text-center mb-6 text-teal-500">PREDICTION MARKETS TODAY</h2>

            {markets.map((market, i) => {
              const yesPercent = market.poolYes && market.poolNo
              ...
              return (
                <div key={i} className="bg-[#00423b] text-white rounded-xl p-4 mb-5">
                  <p className="text-lg font-semibold mb-1">{market.question}</p>
                  <p className="text-sm mb-3">⏳ {getTimeRemainingText(market.eventTime)}</p>

                  {/* Tags from eventType */}
                  <div className="flex flex-wrap gap-2 mb-3">
                    {(market.eventType?.split(",") || []).map((tag: string, j: number) => (
                      <span key={j} className="bg-white text-black text-xs font-semibold px-3 py-1 rounded-full">
                        {tag.trim()}
                      </span>
                    ))}
                  </div>
